...
// 如果一个递归式形如 f(n)=∑i=1maif(n−i)，即齐次线性递推式，我们就可以把数列的递推关系转化为矩阵的递推关系，即构造出一个矩阵的 n 次方乘以一个列向量得到一个列向量，这个列向量中包含我们要求的 f(n)。一般情况下，形如 f(n)=∑i=1maif(n−i) 可以构造出这样的 m×m 的矩阵：
...
// x^2=x+1
// 求得 x1=(1+√5)/2 ，x2=(1−√5)/2 ，设通解为 f(n)=c1x1^n+c2x2^n ，代入初始条件 f(1)=1，f(2)=1，得 c1=1/√5 ，c2=−1/√5 ，我们得到了这个递推数列的通项公式：
...
// n 比较小的时候，可以直接使用过递归法求解，不做任何记忆化操作，时间复杂度是 O(2n)，存在很多冗余计算。
