...
//  1 <= n <= 100
//  1 <= mat[i][j] <= 100
#include "LeetCode1572.hpp"
int Solution::diagonalSum(vector<vector<int>> &mat)
{
	int result = 0;

	for (size_t i = 0; i < mat.size(); i++)
		result = result + mat[i][i] + mat[i][mat.size() - i - 1];
...
// 方法二：枚举对角线元素
// 思路与算法 
// 逐行遍历，记当前的行号为 i，则当前行中处于对角线的元素为： 坐标(i, i) 和坐标(i, n−i−1)，因此我们把(i, i) 与(i, n−i−1) 处的数字加入到答案中。 如果 n 是奇数的话，则主对角线与副对角线存在交点(⌊n2⌋, ⌊n2⌋)，该点会被计算两次。所以当 n 为奇数的时候，需要减掉交点处的值。
// class Solution {
// public:
// 	int diagonalSum(vector<vector<int>>& mat) {
