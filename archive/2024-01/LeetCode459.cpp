...
bool Solution::repeatedSubstringPattern(string s)
{
	...
	for (size_t i = 1; i <= s.size() / 2; i++)
	{
		if (s.size() % i != 0)
			continue;
		idx = i;
		while (idx < s.size() && s.substr(0, i) == s.substr(idx, i))
		{
			idx += i;
		}
		if (idx == s.size())
...
// 思路与算法
// 我们可以把字符串 s 写成
// s′s′⋯s′s′ 的形式，总计 n/n′ 个 s′ 。但我们如何在不枚举 n′ 的情况下，判断 s 是否能写成上述的形式呢？
...
// 我们设 s 在 t 中的起始位置为 i，i∈(0, n)。也就是说，t 中从位置 i 开始的 n 个连续的字符，恰好就是字符串 s。那么我们有：
// s[0:n−1] = t[i:n + i−1]
...
// 那么所有满足 j0 = j + k⋅i 的位置 j0 都有 s[j] = s[j0]，j 和 j0 在模 i 的意义下等价。由于我们已经在模 n 的意义下讨论这个问题，因此 j 和 j0 在模 gcd(n,i) 的意义下等价，其中 gcd 表示最大公约数。也就是说，字符串 s 中的两个位置如果在模 gcd(n,i) 的意义下等价，那么它们对应的字符必然是相同的。
// 由于 gcd(n,i) 一定是 n 的约数，那么字符串 s 一定可以由其长度为 gcd(n,i) 的前缀重复 n/gcd(n,i) 次构成。
...
// 在「正确性证明」部分，如果我们设 i 为最小的起始位置，那么一定有 gcd(n,i) = i，即 n 是 i 的倍数。这说明字符串 s 是由长度为 i 的前缀重复 n/i 次构成；
