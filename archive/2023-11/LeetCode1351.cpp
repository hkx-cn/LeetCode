...
//1.遍历矩阵的每一行。
//
//2.二分查找到该行从前往后的第一个负数，考虑第 i 行，我们记这个位置为 posi，那么第 i 行[posi, m−1] 中的所有数都是负数，所以这一行对答案的贡献就是 m−1−posi + 1 = m−posi 。
//
//3.最后的答案就是 ∑ i = 0 n−1 (m−posi)。
//class Solution {
//public:
//	int countNegatives(vector<vector<int>>& grid) {
//		int num = 0;
...
//空间复杂度：O(1)。
//方法三：分治
//方法二其实只利用了一部分的性质，即每一行是非递增的，但其实整个矩阵是每行每列均非递增，这说明了一个更重要的性质：每一行从前往后第一个负数的位置是不断递减的，即我们设第 i 行的第一个负数的位置为 posi ，不失一般性，我们把一行全是正数的 pos 设为 m，则
//
//pos0 >= pos1 >= pos2 >= ... >= posn−1
//所以我们可以依此设计一个分治算法。
//
//我们设计一个函数 solve(l, r, L, R) 表示我们在统计[l, r] 行的答案，第[l, r] 行 pos 的位置在[L, R] 列中，计算[l, r] 的中间行第 mid 行的的 posmid，算完以后根据之前的方法计算这一行对答案的贡献。然后根据我们之前发现的性质，可以知道[l, mid−1] 中所有行的 pos 是大于等于 posmid，[mid + 1, r] 中所有行的 pos 值是小于等于 posmid的，所以可以分成两部分递归下去，即：
//
//solve(l, mid−1, posmid, R)
//和

//solve(mid + 1, r, L, posmid)
//所以答案就是 m−posmid + solve(l, mid−1, posmid, R) + solve(mid + 1, r, L, posmid)。
...
//
//方法四：倒序遍历
//考虑方法三发现的性质，我们可以设计一个更简单的方法。考虑我们已经算出第 i 行的从前往后第一个负数的位置 posi，那么第 i + 1 行的时候，posi+1 的位置肯定是位于[0, posi] 中，所以对于第 i + 1 行我们倒着从 posi 循环找 posi+1 即可，这个循环起始变量是一直在递减的。
//class Solution {
//public:
//	int countNegatives(vector<vector<int>>& grid) {
