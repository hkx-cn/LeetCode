...
// 方法一：贪心 + 排序
// 不失一般性，我们假设三角形的边长 a,b,ca,b,ca,b,c 满足 a≤b≤ca \leq b \leq ca≤b≤c，那么这三条边组成面积不为零的三角形的充分必要条件为 a+b>ca+b>ca+b>c。
...
// 时间复杂度：O(Nlog⁡N)O(N \log N)O(NlogN)，其中 NNN 是数组 AAA 的长度。

// 空间复杂度：Ω(log⁡N)\Omega(\log N)Ω(logN)。