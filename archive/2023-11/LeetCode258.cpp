...
//};
//复杂度分析
//时间复杂度：O(log⁡num)，其中 num 是给定的整数。对于 num 计算一次各位相加需要 O(log⁡num) 的时间，由于 num≤2^31−1，因此对于 num 计算一次各位相加的最大可能结果是 82，对于任意两位数最多只需要计算两次各位相加的结果即可得到一位数。
//空间复杂度：O(1)。
//
//方法二：数学
//思路和算法
//假设整数 num 的十进制表示有 n 位，从最低位到最高位依次是 a0 到 an−1，则 num 可以写成如下形式：
//num = ∑ i = 0 n−1 ai×10^i = ∑ i = 0 n−1 ai×(10^i −1 + 1) = ∑ i = 0 n−1 ai×(10^i−1) + ∑ i = 0 n−1 ai
