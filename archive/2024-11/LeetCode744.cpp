// 744. 寻找比目标字母大的最小字母
// 给你一个字符数组 letters，该数组按非递减顺序排序，以及一个字符
// target。letters 里至少有两个不同的字符。 返回 letters 中大于 target
// 的最小的字符。如果不存在这样的字符，则返回 letters 的第一个字符。
//
// 示例 1：
...
// letters 最少包含两个不同的字母
// target 是一个小写字母
#include "LeetCode744.hpp"

char nextGreatestLetter(char* letters, int lettersSize, char target) {
    int left = 0, right = lettersSize - 1, mid = -1;
    while (left < right) {
...
//     // 二分查找
//     // 值范围[2,10^4]，指针范围[0,letters.size()-1]
//     int left = 0, right = letters.size() - 1, mid = -1, ans = letters[0];
//     while (left <= right) {
//         mid = left + (right - left) / 2;
//         // 大于target的最小字符
//         // 小于等于target部分舍弃
...
// };
// 复杂度分析
// 时间复杂度：O(n)，其中 n 是列表 letters
// 的长度。需要遍历列表一次寻找比目标字母大的最小字母。 空间复杂度：O(1)。
//
// 方法二：二分查找
...
// public:
//     char nextGreatestLetter(vector<char>& letters, char target) {
//         return target < letters.back() ? *upper_bound(letters.begin(),
//         letters.end() - 1, target) : letters[0];
//     }
// };
// 复杂度分析
// 时间复杂度：O(logn)，其中 n 是列表 letters 的长度。二分查找的时间复杂度是
// O(logn)。 空间复杂度：O(1)。
