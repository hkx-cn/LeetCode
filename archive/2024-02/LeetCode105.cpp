...
TreeNode* Solution::buildTree(vector<int>& preorder, vector<int>& inorder){
    ...
    TreeNode* head = new TreeNode(preorder[0]), *temp = nullptr;
    stack<TreeNode*> stack;
    stack.push(head);
    for (size_t i = 1; i < map.size(); i++) {
        if (map[stack.top()->val] > map[preorder[i]]) {
            stack.top()->left = new TreeNode(preorder[i]);
            stack.push(stack.top()->left);
        }
        else {
            int min = 10000000, temp = 0;
            for (size_t j = 0; j < i; j++) {
                temp = map[preorder[i]] - map[preorder[j]];
                if (temp > 0) {
                    min = min < temp ? min : temp;
                }
            }
            while (!stack.empty()) {
                if (map[preorder[i]] - map[stack.top()->val] == min) {
                    stack.top()->right = new TreeNode(preorder[i]);
